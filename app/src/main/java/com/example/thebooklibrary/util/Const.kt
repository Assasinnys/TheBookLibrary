package com.example.thebooklibrary.util

import com.example.thebooklibrary.R

const val BASE_URL = "https://powerful-citadel-31931.herokuapp.com"

const val BOOK_PREF = "book_pref"

const val BOOK_DATABASE = "book_database"
const val BOOK_TABLE = "books"

const val NO_ERROR = R.string.no_error
const val ERR_EMPTY_FIELD = R.string.error_empty_field
const val ERR_PASS_DIFF = R.string.error_pass_not_eq
const val ERR_INVALID_EMAIL = R.string.error_invalid_email
const val ERR_PASS_TOO_SHORT = R.string.error_incorrect_pass
const val ERR_ALREADY_EXIST = R.string.error_already_exist
const val CONNECTION_ERROR = "connection error" // TODO change to string res


const val AUTHORIZATION_HEADER = "Authorization"